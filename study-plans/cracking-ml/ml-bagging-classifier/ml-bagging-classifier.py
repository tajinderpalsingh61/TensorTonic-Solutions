...
def bagging_classify(X_train, y_train, X_test, n_estimators=10, max_depth=5, seed=42):
    ...
    for i in range(n_estimators):
        samples = np.unique(rng.randint(0, n, size=n))
        X = X_train[samples]
        y = y_train[samples]
        tree = build_tree(X, y, 0)
        predictions.append([predict_one(tree, x) for x in X_test])

    predictions = np.array(predictions)
    
    out = np.apply_along_axis(lambda col: np.argmax(np.bincount(col)), axis=0, arr=predictions)
